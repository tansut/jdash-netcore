                    ...
                    if (Convert.ToInt32(command.ExecuteScalar().ToString()) != 2)
                    {
                        using (var transaction = connection.BeginTransaction(System.Data.IsolationLevel.ReadUncommitted))
                        {
                            try
                            {
                                ...
                                dashboardCreateCommand.CommandText = string.Format(Scripts.SQLScripts.DashboardTableCreationScript, this.defaultScheme);
                                ...
                                dashletCreateCommand.CommandText = string.Format(Scripts.SQLScripts.DashletTableCreationScript, this.defaultScheme);
